...
(function (Drupal, drupalSettings) {
  Drupal.behaviors.ucl_modal = {
    attach: function (context, settings) {
      const modalElement = document.getElementById('ucl-modal'); // Get the the modal element

      if (modalElement) {
        window.onload = function () {
          const lastShownKey = 'modalLastShown'; // Key to store the last shown date in localStorage
          const daysToShowAgain = 30; // Number of days before showing the modal again
          ...
          if (
            !lastShown ||
            now - lastShown > daysToShowAgain * 24 * 60 * 60 * 1000
          ) {
